...
contract NetworkConfig is Script {
    ...
    constructor() {
        if (block.chainid == 1 && vm.activeFork() == 0) {
            // Real mainnet deployment
            activeNetworkConfig = getMainnetEthConfig();
        } else if (block.chainid == 1 && vm.activeFork() != 0) {
            // We are running on a mainnet fork (for testing)
            activeNetworkConfig = getForkEthConfig();
        } else {
            // Local test / Anvil
            activeNetworkConfig = getOrCreateAnvilEthConfig();
        }
    ...
    // === Mainnet fork config ===
    function getForkEthConfig() public pure returns (Config memory) {
        // identical to mainnet, just explicit for clarity
        return getMainnetEthConfig();
    }

    // === Local test mocks (Anvil) ===
